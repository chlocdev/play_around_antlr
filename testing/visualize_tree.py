'''
python path/to/visualize_tree.py path/to/input/file
'''
import sys
from antlr4 import *
from ExprLexer import ExprLexer as Lexer
from MyExprParser import ExprParser as Parser
...
def traverse_tree(dot, tree):
    ...
    if tree.getChildCount() > 0:
        # Get parent string
        parent_str = get_node_name(tree)  + "\n[" + str(id(tree)) + "]" # get unit ID get unique ID of object
        for child in tree.children:
            # Get child string
            child_str = get_node_name(child)  + "\n[" + str(id(child)) + "]" # get unit ID of object
...
def visualize_parse_tree(input_string)->None:
    ...
    tree = parser.program() # Custom Here
